...
public partial class TestGrammarView : ContentPage
{
    ...
    private readonly Queue<Grammar> _remaining = new();
    ...
    private async Task LoadCardsAsync()
    {
        ...
        {
            _cards = [];
            ShowDone("문법 카드를 불러오지 못했어요. 잠시 후 다시 시도해 주세요.");
        }
        finally
        {
            _isLoading = false;
    ...
    private void ShowCard()
    {
        ...
        {
            ShowDone("헷갈린 문법까지 모두 다시 확인했어요.");
            return;
        }

        var grammar = _remaining.Peek();
        titleLabel.Text = grammar.Title;
        categoryLabel.Text = string.IsNullOrWhiteSpace(grammar.Category) ? "GRAMMAR" : grammar.Category.ToUpperInvariant();
        explanationLabel.Text = grammar.Explanation;
        exampleLabel.Text = grammar.Example;
        noteLabel.Text = grammar.Note;
        noteBorder.IsVisible = !string.IsNullOrWhiteSpace(grammar.Note);
    ...
    private async Task GradeAsync(bool correct, string status)
    {
        ...
        if (_session.IsLoggedIn)
        {
            try
            {
                await _api.UpsertGrammarProgressAsync(_session.CurrentUserId!, card.Id, correct, status);
            }
            catch
            {
    ...
    private async Task<Dictionary<string, GrammarProgress>> LoadProgressMapAsync()
    {
        ...
        try
        {
            ...
            return list
                .GroupBy(progress => progress.GrammarId)
